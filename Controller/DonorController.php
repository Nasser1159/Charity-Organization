<?php
// File: Controller/DonorController.php
...
class DonorController {
    ...
    function myaccount($id) {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $donorModel->getById($id);

            // Determine the donation state
            if ($donorModel->hasDonated($id)) {
                $donationState = new DonatedState();
            } else {
                $donationState = new NotDonatedYetState();
            }

            // Create the user object
            $user = new User($id, $donorModel->getUserName(), $donationState);

            // Display the donation status
            $this->donorView->ShowDonorDetails($donorModel, $user);
        }
    }
